      ...
      `}>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-8 space-y-8 custom-scrollbar scroll-smooth">
          <div className="space-y-3">
            <h3 className={`px-2 text-[10px] font-black uppercase tracking-[0.2em] ${subTextColor}`}>Spatial Environments ({settings.customWallpapers.length}/6)</h3>
            <div className={`rounded-3xl p-4 shadow-sm space-y-4 ${isLight ? 'bg-white border border-zinc-200' : 'bg-zinc-800/60'}`}>
              ...
              <div className="flex gap-2 pt-2">
                <button onClick={() => setShowGallery(true)} disabled={settings.customWallpapers.length >= 6} className={`flex-1 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-20 ${isLight ? 'bg-zinc-100 border border-zinc-200' : 'bg-white/5 border border-white/5'}`}>Curated</button>
                <div className="relative flex-[2]">
                  <input type="text" value={wallpaperUrl} onChange={(e) => setWallpaperUrl(e.target.value)} placeholder="Paste URL..." className={`w-full py-3 px-4 rounded-2xl text-[10px] font-bold outline-none border transition-all ${isLight ? 'bg-zinc-50 border-zinc-200 text-zinc-900' : 'bg-black/20 border-white/5 text-white'}`} />
          ...
          <div className="space-y-3">
            <h3 className={`px-2 text-[10px] font-black uppercase tracking-[0.2em] ${subTextColor}`}>Appearance</h3>
            <div className={`rounded-3xl overflow-hidden shadow-sm ${isLight ? 'bg-white border border-zinc-200' : 'bg-zinc-800/60'}`}>
              ...
              <div className="p-5 space-y-4">
                <span className="text-base font-bold block">Accent Theme</span>
          ...
          <div className="space-y-3">
            <h3 className={`px-2 text-[10px] font-black uppercase tracking-[0.2em] ${subTextColor}`}>Developer Tools</h3>
            <div className={`rounded-3xl p-5 space-y-4 shadow-sm ${isLight ? 'bg-white border border-zinc-200' : 'bg-zinc-800/60'}`}>
               ...
               >
                 {isExporting ? (
                   <span className="text-[10px] font-black uppercase tracking-widest animate-pulse">Compiling Archive...</span>
                 ) : (
                   <>
                     <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="12" y1="18" x2="12" y2="12"/><polyline points="9 15 12 12 15 15"/></svg>
                     <span className="text-[10px] font-black uppercase tracking-widest">Export Source (PDF)</span>
                   </>
                 )}
               </button>
               <p className="text-[9px] opacity-30 font-bold uppercase tracking-[0.1em] text-center">Generates a local technical archive of all project files.</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className={`relative w-full max-w-xs rounded-[36px] border p-8 shadow-2xl animate-insight-pop ${isLight ? 'bg-white border-zinc-200' : 'bg-[#1a1a1e] border-white/10'}`}>
             <div className="space-y-6">
               ...
               <div className="grid grid-cols-1 gap-3">
                 <button onClick={() => replaceInputRef.current?.click()} className={`py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all active:scale-95 border ${isLight ? 'bg-zinc-100 border-zinc-200 text-zinc-900' : 'bg-white/5 border-white/10 text-white'}`}>Replace from Gallery</button>
                 <button onClick={() => removeWallpaper(modifyingIndex)} className={`py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all active:scale-95 bg-red-500/10 text-red-500`}>Delete Slot</button>
               </div>
              ...
              {CURATED_SCENIC.map((item, i) => (
                <div key={i} onClick={() => addCurated(item)} className="group relative aspect-[4/5] rounded-3xl overflow-hidden cursor-pointer active:scale-95 transition-all shadow-md">
                  ...
                  <div className="absolute bottom-3 left-3 right-3">
                    <span className="text-[10px] font-black uppercase tracking-tight text-white block truncate">{item.header}</span>
                  </div>
                </div>
              ))}
            </div>
